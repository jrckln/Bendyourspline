      ...
      <b>Setup:</b>  Go to "Data options" and select "No data" from the dropdown menu. Make sure to reset the inputs at the bottom of the app.',
        ...
        paste0(
          ...
          ' to 1 and observe the effect on the response function.'
            ...
            paste0('Set the second power to 2 and set the ', colorize('associated coefficient', col[2]) ,' to 1! You obtain x + x<sup>2</sup>, i.e. the first and the second fractional polynomial are summed-up.'), 
            paste0('Now move ', colorize('coefficient 1', col[1]) , ' to -1 and observe the effect on the response function.'), 
            paste0('Set ', colorize('coefficient 1', col[1]) , ' to +1 and ', colorize('coefficient 2', col[2]) , ' to -1!'), 
            ...
            this case the second term is also multiplied with log(x). (Check the formula in the "Formula" panel.)', 
        ...
        "Advanced I" = c(
          ...
          <b>Setup:</b>  Go to "Data options" and select the variable pair "Diastolic blood pressure ~ Age" from the dropdown menu. In advanced settings, set sample size to 100% and sex to "both". Make sure to reset the inputs and enable data points and  ', colorize('LOESS smoother', loesscol) , ' at the bottom of the app.'),
           paste0('The panel "Response function" shows the current response function, a simple horizontal line. Try to move the response function up to where the ', colorize('LOESS smoother', loesscol) , ' is. Move the slider of  the intercept such that the response is close to the ', colorize('LOESS smoother', loesscol) , '.'), 
            paste0('By default, the two powers are set to 1. Increase the coefficient range, and move the slider of ', colorize('coefficient 1', col[1]) , ' and the intercept to improve the fit. Goodness of fit can be monitored with the adjusted R<sup>2</sup> values displayed in the "Goodness of fit" panel. Move the sliders of ', colorize('coefficient 1', col[1]) , ' and the intercept such that you obtain an adjusted R<sup>2</sup> of at least 0.08. <br>
                   Hint: You can use the + and - buttons to fine-tune the coefficient and intercept values.'),
            paste0('Obviously, there is curvature in the ', colorize('LOESS smoother', loesscol) , ', which cannot be appropriately modelled with a linear response function as obtained in the previous step. Now, set the second power to 2. You can now increase the curvature by moving the slider of ', colorize('coefficient 2', col[2]) , ' into the negative range. Hint: enlarge the coefficient range ("Coefficient range: +"). There is also a feature to compute an optimal intercept automatically by clicking "Adjust Intercept automatically". <br>
        ...
        <b>Setup:</b>  Go to "Data Options" and select the variable pair "Bmi ~ Age" from the dropdown menu. In advanced settings, set sample size to  10%, sex to "male" and the seed to 100. By doing so, a random sample with 10% of the full data set is drawn. Reset the inputs and display the values by enabling data points option at the bottom.',
          ...
          paste0('Is it possible to receive a similar shape like the LOESS smoother? Enable ', colorize('optimal fit', optfitcol) , '. Is your solution close to the optimal fit?'),
          ...
          paste('Are the response functions for women and men very different? <br>
        Hint: The optimal response function for men is currently shown in the "Response function" panel. Select "female" in "Data options" and compare the ', colorize('optimal fit', optfitcol), ' of women to the response function, which was fitted on data from men.')
  ...
  'bs' = list(
    'Basic' = c(
      ...
      paste0(
        ...
        ' by 1, set ',
        colorize('coefficient 1', col[1]) ,
        ' to 1 and observe the effect on the response function. Afterwards, set ', colorize('coefficient 1', col[1]), ' back to 0.'
      ),
      paste0(
    ...
    'Advanced II' = c(
      ...
      paste('You probably found that your fit is quite robust. Are the response functions for women and men very different? <br>
      Hint: Select "male" in "Data options" and click "set optimal fit" to receive the response function for men. Then select "female" in "Data options" and compare the ', colorize('optimal fit', optfitcol), ' of women to the response function, which was fitted on data from men.')
    )
  )
,
'nsp' = list(
  'Basic' = c(
    ...
    paste('Create a &Cup; -shaped response function by playing around with ',  
          colorize('coefficient 1', col[1]), 
          ', ', 
          colorize('coefficient 2', col[2]),
          ' and ', 
          colorize('coefficient 3', col[3]) , ' only choosing values from {-1,-0.5,0, 0.5,1}.'),
    paste('Create a &Cap; -shaped response function by playing with ', 
          colorize('coefficient 1', col[1]), 
          ', ', 
          colorize('coefficient 2', col[2]),
          ' and ', 
  ...
  'Advanced II' = c(
    ...
    paste0('Click again on "set optimal fit" such that the fit is adapted to the smaller data set. Then change the seed in "Data options" to 100. Does your response function based on the previous data sample still fits well? Compare your previous response function to the current ', colorize('optimal fit', optfitcol) , '. Also look at the adjusted R<sup>2</sup> and the maximum adjusted R<sup>2</sup>.'),
